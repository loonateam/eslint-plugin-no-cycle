...
ExportMap.get = function (source, context, aliases = {}) {
  const path = resolve(source, context);

  if (path == null) {
    const partsOfSource = source.split('/');
    const key = partsOfSource[0];
    const keyWithStar = `${key}/*`;

    const value = aliases[key] || aliases[keyWithStar];
    if (Array.isArray(value) && value.length) {
      const resolvedSource = `${process.cwd()}/${value[0].replace(/\*$/, partsOfSource.slice(1).join('/'))}`;
      const resolvedPath = resolve(resolvedSource, context);
